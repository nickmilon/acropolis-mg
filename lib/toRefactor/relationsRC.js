...
class MgRel {
  ...
  relativesArr(id, pORc = 'p', { pagesize, pagenext, pageprev, sort } = {}) {
    ...
    const options = () => {
      ...
      if (sort !== undefined) {
        ...
        opt.sort = { [idC]: 1 }; // ❕: sort can fuck up with projection
      }
  ...
  relativesGetQuery(id, pORc = 'p', pagenext = null, pageprev = null, pagesize = null) {
    ...
    const options = () => {
      ...
      // ❕: no need to sort it comes out sorted coz of index used
...
class FollowRel extends MgRel {
   ...
   * ❕  use it rarely during the transition period and only get the information you actually need from
